...
def _append_signature_metadata(pdf_bytes: bytes, sig_data: dict) -> bytes:
    try:
        sig_footer = (
        ...
        result = pdf_bytes + sig_footer.encode('ascii')
...
async def generate_report(db, user_id: int):
    ...
    try:
        ...
        sig_data = sig_service.sign_pdf(pdf_bytes)
        if sig_data:
            pdf_bytes = _append_signature_metadata(pdf_bytes, sig_data)
